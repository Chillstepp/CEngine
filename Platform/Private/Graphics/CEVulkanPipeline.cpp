...
namespace CE{
	...
	CEVulkanPipelineLayout::CEVulkanPipelineLayout(CEVulkanLogicDevice* device,
	...
	CEVulkanPipelineLayout::~CEVulkanPipelineLayout()
	{
	...
	VkResult CEVulkanPipelineLayout::CreateShaderModule(const std::string& filePath, VkShaderModule* outShaderModule)
	{
	...
	CEVulkanPipeline::CEVulkanPipeline(CEVulkanLogicDevice* device,
		CEVulkanRenderPass* renderPass,
		CEVulkanPipelineLayout* pipelineLayout) :  mDevice(device), mRenderPass(renderPass), mPipelineLayout(pipelineLayout)
	{
	...
	void CEVulkanPipeline::Create()
	{
		...
		VkPipelineVertexInputStateCreateInfo vertexInputStateInfo = {
			...
			.pVertexBindingDescriptions = mPipelineConfig.vertexInputState.vertexBindings.data(), //描述顶点缓冲区中数据的绑定方式和步长
			.vertexAttributeDescriptionCount = static_cast<uint32_t>(mPipelineConfig.vertexInputState.vertexAttributes.size()),
			.pVertexAttributeDescriptions = mPipelineConfig.vertexInputState.vertexAttributes.data() //描述每个顶点属性的格式、位置和偏移量
		};
		...
		VkGraphicsPipelineCreateInfo pipelineInfo = {
			...
			.pNext = nullptr,
			.flags = 0,
			.stageCount = ARRAY_LENGTH(shaderStageInfo),
	...
	void CEVulkanPipeline::Bind(VkCommandBuffer cmdBuffer)
	{
		vkCmdBindPipeline(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mHandle);
	}
	CEVulkanPipeline* CEVulkanPipeline::SetVertexInputState(const std::vector<VkVertexInputBindingDescription>& vertexBindings,
	...
	CEVulkanPipeline* CEVulkanPipeline::SetInputAssemblyState(VkPrimitiveTopology topology,
	...
	CEVulkanPipeline* CEVulkanPipeline::SetRasterizationState(const PipelineRasterizationState& rasterizationState)
	{
	...
	CEVulkanPipeline* CEVulkanPipeline::SetMultisampleState(VkSampleCountFlagBits samples,
	...
	CEVulkanPipeline* CEVulkanPipeline::SetDepthStencilState(const PipelineDepthStencilState& depthStencilState)
	{
	...
	CEVulkanPipeline* CEVulkanPipeline::SetColorBlendAttachmentState(VkBool32 blendEnable,
	...
	CEVulkanPipeline* CEVulkanPipeline::SetDynamicState(const std::vector<VkDynamicState>& dynamicStates)
	{
		mPipelineConfig.dynamicState.dynamicStates = dynamicStates;
		return this;
	}
	CEVulkanPipeline* CEVulkanPipeline::EnableAlphaBlend()
	{
	...
	CEVulkanPipeline* CEVulkanPipeline::EnableDepthTest()
	{
